...
class Host():
    def __init__(
        self, owner, name, port, counter_limit, token, chat_id,
    ...
    def goto_state_failed(self):
        message = (
            "{} Your server isn't responding properly.\
            Please check it: {}".format(self.owner, self.name))
        sendMessage(self.token, self.chat_id, message)
        self.state = State.failed
        self.recheck_at = (datetime.datetime.now() +
                           datetime.timedelta(**self.recheck_duration_para))
    ...
    def goto_state_running(self):
        logging.debug("Logging state is turned on running again for %s",
                      self.name)
        self.state = State.running
        self.counter = 0
...
def main():
    ...
    parser.add_argument("-chec", "--recheck_duration",
                        dest="recheck_duration", default="days:1",
                        help="set the time until recheck")
    ...
    parser.add_argument("-c", "--counter", dest="counter", default=5,
                        type=int, help="set the check times,\
                        until the msg will be sent")
    ...
    for host in args.hosts.split(","):
        ...
        hosts.append(Host(owner, name, port, counter_limit, token, chat_id,
                          recheck_duration_para))
